...
void addRecChi2Options(ActsExamples::Options::Description& desc) {
  ...
  opt("chi2-truth-seeds", bool_switch(), "Use truth seeds for steering Chi2");
  opt("number-of-iterations", value<unsigned int>()->default_value(1), "Number of update steps (ignored for now)");
}
...
int runRecChi2Tracks(int argc, char* argv[],
  ...
  bool truthSeeded = vm["chi2-truth-seeds"].template as<bool>();
  ...
  // Setup the track finding algorithm with Chi2
  ...
  // since the unselected Chi2 track might have a majority particle not in the
  ...
  trackStatesWriter.outputFilename = "trackstates_chi2.root";
  trackStatesWriter.outputTreename = "trackstates_chi2";
  sequencer.addWriter(std::make_shared<RootTrajectoryStatesWriter>(
      trackStatesWriter, logLevel));

  // write track parameters from Chi2
  ...
  trackParamsWriter.outputFilename = "trackparams_chi2.root";
  trackParamsWriter.outputTreename = "trackparams_chi2";
  sequencer.addWriter(std::make_shared<RootTrajectoryParametersWriter>(
      trackParamsWriter, logLevel));

  // Write Chi2 performance data
  // TODO: create special Chi2 performance writer?
