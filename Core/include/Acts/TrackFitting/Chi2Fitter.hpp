...
class Chi2Fitter {
    ...
    Result<void> processSurface(const Surface* surface, propagator_state_t& state,
      ...
      if (sourcelink_it != inputMeasurements.end()) {
        ...
        const Acts::BoundMatrix fullJacobian = result.previousJacobian * jacobian; // TODO: here, `jacobian` is the jacobian between the previous and the current bound state.
              ...
              // TODO: Can we just multiply all jacobians? Or should we take some sort of 'transport matrix'?
 ...
 public:
  ...
  auto fit(const std::vector<source_link_t>& sourcelinks,
    ...
    // TODO: does Eigen optimize inverse() for diagonal matrices?

    // XXXX ActsDynamicMatrix covarianceInverse = (1 / covariance.array()).matrix(); // use .array() for element-wise operations (note: doesn't work because of 0-entries)
