  ...
  class Actor {
   public:
    ...
    void operator()(propagator_state_t& state, const stepper_t& stepper,
      ...
      ACTS_VERBOSE("KalmanFitter step s=" << result.smoothed << "r=" << result.reversed);
      ...
      if (surface != nullptr) {
        ...
        if (result.reversed) {
          ACTS_VERBOSE("Perform " << direction << " filter step (reversed)");
      ...
      ACTS_INFO("KF after filter steps.");
      ...
      ACTS_INFO("KF marker 2: end of operator()");
    }
   ...
   public:
    ...
    bool operator()(propagator_state_t& state, const stepper_t& /*stepper*/,
      ...
      if (!result.result.ok() or result.finished) {
        ACTS_INFO("KF Aborter: return true");
        return true;
      }
      return false;
    }
           ...
           const Chi22FitterOptions<calibrator_t, outlier_finder_t>& kfOptions)
{
    ...
    ACTS_VERBOSE("Preparing " << sourcelinks.size() << " input measurements in regular fit()");
    std::map<GeometryIdentifier, source_link_t> inputMeasurements;
